...
namespace Maze
{
    public class MazeRecImprovments : IMapProvider
    {
        ...
        public MazeRecImprovments(int? seed)
        {
            if (seed != null)
            {
        ...
        private void Walk(MapVector currentVector)
        {
            ...
            // CHANGE: Run the possible direction checks in parralel --> MASSIVE IMPROVEMENT
            Parallel.ForEach(possibleDirections, dir =>
            {
                var nextVector = currentVector + (MapVector)dir;
                var oppositeDir = GetReverseDirection(dir);

                if ((nextVector.X >= 0 && nextVector.X < this._gridWidth && nextVector.Y >= 0 && nextVector.Y < this._gridHeight) && !_visited.Contains(nextVector))
                {
